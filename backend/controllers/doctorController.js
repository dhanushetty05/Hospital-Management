...
export const createDoctor = async (req, res) => {
  try {
    ...
    return res.status(201).json({ success: true, data: out, doctor: out });
  } catch (err) {
...
export const getDoctorById = async (req, res) => {
  try {
    ...
    const normalized = normalizeDocForClient(doctor.toObject());
    return res.json({ success: true, data: normalized, doctor: normalized });
  } catch (err) {
...
export const deleteDoctor = async (req, res) => {
  try {
    ...
    if (doctor.imagePublicId) {
      await deleteFromCloudinary(doctor.imagePublicId).catch(e => console.warn("Cloudinary delete warning:", e));
    }
